...
@implementation ZKVerifyAlertView
...
- (void)updateLayout:(BOOL)animated{
    ...
    if (animated) {
        [UIView animateWithDuration:0.2 animations:^{
            weakSelf.contentView.frame = CGRectMake(contentViewX, contentViewY, contentViewW, contentViewH);
        }];
    } else {
...
- (void)performCallback
{
    if (!_isVerified) {
        if (_remainingVerifyNumber == 0) {
            ...
            _state = ZKVerifyStateNot; // 未开始验证
        } else {
            _state = ZKVerifyStateIncomplete; // 未完成验证
        }
    } else {
        _state = ZKVerifyStateSuccess; // 验证成功
    }
    ...
    if([keyPath isEqualToString:@"tracking"]) {
        ...
        [_puzzleView checkVerificationResults:^(BOOL isVerified) {
            ...
            if (isVerified) {
                ...
                weakSelf.remainingVerifyNumber --;
                weakSelf.resultImageView.hidden = NO;
                weakSelf.resultImageView.image = [UIImage imageNamed:@"ic_失败图标"];
                
                if (weakSelf.remainingVerifyNumber <= 0) {
                    weakSelf.slider.enabled = NO;
                    weakSelf.puzzleView.enable = NO;
                    weakSelf.refreshButton.enabled = NO;
                    weakSelf.messageLabel.text = [NSString stringWithFormat:@"您已3次验证失败，将不能继续操作！"];
                } else {
                    weakSelf.slider.enabled = NO;
                    weakSelf.puzzleView.enable = NO;
                    weakSelf.messageLabel.text = [NSString stringWithFormat:@"验证失败，您还有%zd次机会！请点击刷新按钮，刷新验证码。", weakSelf.remainingVerifyNumber];
                }
