...
public extension Date {
    ...
    var weekday: String {
        ...
        return DateFormatter(withFormat: "EEEE", locale: "zh_CN").string(from: self)
    }

    // MARK: 1.11、从日期获取 月(英文)
    /// 从日期获取 月(英文)
    var monthAsString: String {
        return DateFormatter(withFormat: "MMMM", locale: "zh_CN").string(from: self)
    }
}
...
public extension Date {
    ...
    static func timestampToFormatterDate(timestamp: String) -> Date {
        ...
        if timestamp.count == 10 {
            timeSta = TimeInterval(timestamp.toDouble()!)
        }else{
            timeSta = TimeInterval(timestamp.toDouble()! / 1000)
        }
        // 时间戳转为Date
